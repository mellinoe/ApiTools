...
namespace Microsoft.Cci.Extensions
{
    public class TypeDependencies
    {
        ...
        public TypeDependencies(ITypeDefinition type, bool unique = true)
        {
            if (unique)
            {
                this._publicDependents = new HashSet<ITypeReference>();
                this._dependents = new HashSet<ITypeReference>();
                this._methodDependents = new HashSet<IMethodReference>();
            }
            else
            {
                this._publicDependents = new List<ITypeReference>();
                this._dependents = new List<ITypeReference>();
                this._methodDependents = new List<IMethodReference>();
            }
            CalculateForType(type);
        }

        public IEnumerable<ITypeReference> PublicTypeDepdencies { get { return this._publicDependents; } }
        public IEnumerable<ITypeReference> AllTypeDependencies { get { return this._dependents; } }
        public IEnumerable<IMethodReference> MethodDependencies { get { return this._methodDependents; } }
        ...
        {
            if (isPublic)
                this._publicDependents.Add(type);

            this._dependents.Add(type);
        }
