    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            using (TextWriter output = GetOutput())
            {
                ...
                IEnumerable<IAssembly> contractAssemblies =  contractHost.LoadAssemblies(_contractSet, _contractCoreAssembly);
                ...
                IEnumerable<IAssembly> implAssemblies = implHost.LoadAssemblies(contractAssemblies.Select(a => a.AssemblyIdentity), _warnOnIncorrectVersion);

                // Exit after loading if the code is set to non-zero
                if (Environment.ExitCode != 0)
                    return;

                ICciDifferenceWriter writer = GetDifferenceWriter(output);
        ...
        private static ICciDifferenceWriter GetDifferenceWriter(TextWriter writer)
        {
            ...
            IDifferenceFilter filter = new DifferenceFilter<IncompatibleDifference>();
            ...
            if (!string.IsNullOrEmpty(_baselineFileName))
            {
                ...
                filter = new BaselineDifferenceFilter(filter, _baselineFileName);
            }
